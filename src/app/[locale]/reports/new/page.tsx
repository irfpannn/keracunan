...
import { UploadCloud, MapPin, FileText, Loader2, AlertCircle } from 'lucide-react';
...
export default function ReportRestaurantPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save to Firebase
      await submitReport(base64Image, location, remarks);
      
      // Redirect to public feed
      router.push('/reports'); // the layout handles locale automatically or we can rely on next router
    } catch (err: any) {
    ...
    <div className="max-w-2xl mx-auto p-6 mt-8 bg-white rounded-2xl shadow-xl shadow-red-900/5">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">{t('photo_label')}</label>
          <div 
            className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${
              imagePreview ? 'border-gray-200 bg-gray-50' : 'border-red-200 bg-red-50/50 hover:bg-red-50 cursor-pointer'
            }`}
          >
            {imagePreview ? (
              <div className="relative group">
                ...
                >
                  <AlertCircle className="w-4 h-4" /> {/* Use some close icon or keep simple */}
                </button>
              </div>
            ) : (
              <label className="cursor-pointer flex flex-col items-center justify-center">
                <UploadCloud className="w-10 h-10 text-red-500 mb-3" />
                <span className="text-sm font-medium text-gray-700">{t('photo_hint_1')}</span>
                <span className="text-xs text-gray-500 mt-1">{t('photo_hint_2')}</span>
                <input 
                  type="file" 
                  accept="image/*" 
                  className="hidden" 
        ...
        {/* Location */}
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">{t('location_label')}</label>
        ...
        {/* Remarks */}
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">{t('remarks_label')}</label>
          <div className="relative">
