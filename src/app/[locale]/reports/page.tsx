...
import { listenToReports, Report } from '@/lib/firebase/reports';
...
function PublicReportModal({ report, currentStatus }: { report: Report; currentStatus: string; }) {
        ...
        <div className="mt-4 space-y-6">
          ...
          <div className="h-56 w-full bg-gray-100 rounded-xl relative overflow-hidden group">
            ...
            <div className="absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm border bg-white/90 backdrop-blur-sm">
              ...
              <span className="text-gray-700">{tStatus(currentStatus as any) || currentStatus}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-1 flex items-center">
                <MapPin className="w-4 h-4 mr-1.5 text-red-500" /> {t('col_location')}
              </h4>
              <p className="text-sm text-gray-600">{report.location}</p>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-1 flex items-center">
                <Clock className="w-4 h-4 mr-1.5 text-gray-400" /> {t('col_submitted')}
              </h4>
          ...
          <div className="pt-4 border-t border-gray-100">
            <h4 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-4">{t('history')}</h4>
            <div className="space-y-4">
              {report.history?.map((entry, index) => (
                <div key={index} className="flex gap-3 relative">
                  ...
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center z-10 flex-shrink-0 mt-0.5 border-2 ${
                    entry.status === 'Open' ? 'bg-blue-50 border-blue-200 text-blue-500' :
                    entry.status === 'Pending Visit' ? 'bg-amber-50 border-amber-200 text-amber-500' :
                    entry.status === 'Action Taken' ? 'bg-green-50 border-green-200 text-green-500' :
                    entry.status === 'Resolved' ? 'bg-emerald-50 border-emerald-200 text-emerald-500' :
                  ...
                  <div className="flex-1 pb-1">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded border ${getStatusColor(entry.status)} inline-block w-fit`}>
                        {tStatus(entry.status as any) || entry.status}
                      </span>
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 mt-8">
      ...
      {reports.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">{t('no_reports_title')}</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-200 text-xs uppercase tracking-wider text-gray-500">
                  ...
                  <th className="px-6 py-4 font-medium">{t('col_submitted')}</th>
                  <th className="px-6 py-4 font-medium">{t('col_remarks')}</th>
                  <th className="px-6 py-4 font-medium">{t('col_status')}</th>
                  <th className="px-6 py-4 font-medium text-center">{t('col_timeline')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {reports.map((report) => {
                  const currentStatus = report.history && report.history.length > 0 
                    ...
                    <tr key={report.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 align-middle">
                        <div className="flex items-center gap-3">
                          {report.image ? (
                            <img src={report.image} alt="" className="w-10 h-10 rounded-md object-cover flex-shrink-0 border border-gray-100" />
                          ) : (
                      ...
                      <td className="px-6 py-4 align-middle whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${getStatusColor(currentStatus)}`}>
